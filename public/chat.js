$(function() {
	...
	function sendMessage() {
        if (message.val() != "") {
            socket.emit("new_message", { message: message.val().replace(/</g, '&lt;').replace(/>/g, '&gt;') });
	...
	socket.on("new_message", data => {
		...
		chatroom.append(
			"<div class='message-row'><img class='avatar' src=" + data.avatar + "><p class='message'><strong>" +
				data.username +
				"</strong>" +
				"<br>" + data.message +
				"</p><br></div>"
    ...
    socket.on("user_disconnect", (data) => {
        chatroom.append(
            "<p class='message disconnect'><em>" + data.username + " has disconnected</em></p>"
        );
    });
    ...
    function changeUsername(){
        socket.emit("change_username", { username: username.val().replace(/</g, '&lt;').replace(/>/g, '&gt;') });
